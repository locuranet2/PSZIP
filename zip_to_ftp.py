...
class ZipToFtpApp:
    ...
    def _load_i18n(self):
        self.lang_data = {}
        try:
            if os.path.exists("i18n_zip_ftp.json"):
                with open("i18n_zip_ftp.json", "r", encoding="utf-8") as f:
                    self.lang_data = json.load(f)
            else:
                # Fallback minimo si el archivo no existe
                self.lang_data = {"es": {"app_title": "Uploader"}}
